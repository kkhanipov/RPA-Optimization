...
public:
	...
	bool add_sequence(char * sequence, unsigned int _sequence_length, ostream &err_msg = cout); //ability to add a sequence to the array of sequences by giving the actual sequence
...
Array_Sequences::Array_Sequences(char * filename, ostream &err_msg)
{
	...
	// Initialize 
	for (i = 0; i<max_number_of_sequences; i++) {sequence[i] = NULL; }
...
bool Array_Sequences::add_sequence(Sequence * sequence, unsigned int _sequence_length, ostream &err_msg)
{
